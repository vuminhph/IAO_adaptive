...

{
    ...
    protected override void FixedUpdate()
    {
        ...
        if (secondStage == false && health <= GameObject.FindGameObjectWithTag("GM").GetComponent<GameManager>().Boss.health * 0.25f)
        {
            invincible = true;
            secondStage = true;
            spawnEnemy();
            ...
            disableMovement();
            health = GameObject.FindGameObjectWithTag("GM").GetComponent<GameManager>().Boss.health;
            GameObject.Find("Canvas").GetComponent<BossRoomUIController>().bossHPGain();
            animator.CrossFade("StageTransition", 0, 0);
        }
