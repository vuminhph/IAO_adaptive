...
public class GameManager : MonoBehaviour
{
    ...
    [HideInInspector]
    public float minSkillFactor = 0.65f;
    ...
    [HideInInspector]
    public float boringFactor = 0.05f;
    ...
    {
        WWW www = new WWW(webURL + privateCode + "/add/" + WWW.EscapeURL(username) + "/" + score + "/" + (int)Time.time + "/" + "Adaptive");
        yield return www;
    ...
    // {
    //     StartCoroutine(DeleteOldScore(username));
    // }

    // IEnumerator DeleteOldScore(string username)
    // {
    ...
    // {
    //     StartCoroutine(UpdateOldScore(username, score));
    // }

    // IEnumerator UpdateOldScore(string username, int score)
    // {   
    //     yield return StartCoroutine(DeleteOldScore(username));
    //     yield return StartCoroutine(UploadNewScore(username, score));
    // }
    ...
    //             {
    //                 Time.timeScale = 1;
    //                 GameObject.FindGameObjectWithTag("Player").GetComponent<PlayerController>().allowInput = true;
